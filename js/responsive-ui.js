...
function initViewToggle() {
  ...
  if (tableViewBtn && cardViewBtn) {
    tableViewBtn.addEventListener("click", () => {
      document
        ...
        .classList.remove("books-card-view");
    ...
    if (
      savedViewMode === "card" ||
      (window.innerWidth <= 576 && !savedViewMode)
    ) {
      cardViewBtn.click();
    } else {
      tableViewBtn.click();
  ...
  const borrowedBooksContainer = document.querySelector(
    ".borrowed-books-container"
  );
  if (
    borrowedBooksContainer &&
    !borrowedBooksContainer.querySelector(".view-toggle-borrowed")
  ) {
    const viewToggle = document.createElement("div");
    ...
    `;
    borrowedBooksContainer.insertBefore(
      viewToggle,
      borrowedBooksContainer.firstChild
    );
  }

  // Add event listeners to borrowed books view toggle buttons
  const tableViewBorrowedBtn = document.querySelector(
    ".table-view-borrowed-btn"
  );
  ...
  rows.forEach((row) => {
    ...
    card.innerHTML = `
      ...
      <div class="borrowed-book-dates">
        ...
        ${
          dueDate
            ? `<span class="${cells[3].className}">Due: ${dueDate}</span>`
            : ""
        }
      </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (borrowedBooksContainer) {
    borrowedBooksObserver.observe(borrowedBooksContainer, {
      childList: true,
      subtree: true,
    });
  }
